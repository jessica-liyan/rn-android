...
export default class ModifyPayPwd extends Component {
  constructor(props){
    super(props)
    this.state = {
      id:'',
  ...
  // 提交修改的资料（用户名，生日，性别）
  onModify(){
    const {id,payPwd,payPwdTwice,code} = this.state;
    if(!payPwd){
      ToastAndroid.show(`请输入支付密码！`, ToastAndroid.SHORT)
    }else if(!payPwdTwice){
      ToastAndroid.show(`请再次输入支付密码！`, ToastAndroid.SHORT)
    }else if(payPwd !== payPwdTwice){
      ToastAndroid.show(`两次支付密码不一致！`, ToastAndroid.SHORT)
    }else{
      fetch('http://liuwbox.com/zzbao/app/user/resetPay.htm?userId='+id+'&payPwd='+payPwd+'&captcha='+code+'', {
        method: 'post',
        headers: {
  ...
  timeOut(){
    ...
    if(!tel){
      ...
      fetch('http://liuwbox.com/zzbao/app/user/sms.htm?tel='+ tel +'&type=1', {
        method: 'post',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
    ...
    return (
      <View style={styles.login}>
        <View style={styles.loginWrap}>
          ...
          />
          <Image source={require('../image/code.png')} style={styles.loginIcon}/>
        </View>
        <View style={styles.loginWrap}>
          <TextInput
            style={styles.loginInput}
            ...
            secureTextEntry={true}
            keyboardType="numeric"
            onChangeText={(payPwdTwice) => this.setState({payPwdTwice})}
            value={this.state.payPwdTwice}
          />
          <Image source={require('../image/code.png')} style={styles.loginIcon}/>
        </View>
        <View style={styles.loginWrap}>
